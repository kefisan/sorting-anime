...
def insertion_sort(data):
    for i in range(1, len(data)):
        key = data[i]
        j = i - 1
        while j >= 0 and float(data[j][9]) > float(key[9]):
            data[j + 1] = data[j]
            j -= 1
        data[j + 1] = key
    return data
...
def partition(data, low, high):
    ...
    while True:
        i += 1
        while data[i][9] < pivot[9]:
            i += 1
        j -= 1
        while data[j][9] > pivot[9]:
            j -= 1
        if i >= j:
            return j
...
def heapify(data, n, i):
    ...
    left = 2 * i + 1
    right = 2 * i + 2
    if left < n and float(data[i][9]) < float(data[left][9]):
        largest = left
    if right < n and float(data[largest][9]) < float(data[right][9]):
        largest = right
    if largest != i:
...
def counting_sort(data, exp1):
    ...
    count = [0] * 10
    for i in range(n):
        index = (int(float(data[i][9]) * 10) // exp1)
        count[index % 10] += 1
    for i in range(1, 10):
        count[i] += count[i - 1]
...
def radix_sort(data):
    max1 = max(float(row[9]) * 10 for row in data)
    exp = 1
    while max1 // exp > 0:
